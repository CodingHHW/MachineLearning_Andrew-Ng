...
% ƒ‹ π”√sum
for i = 1:m
    J = J + 1/m * (-y_vect(:,i)'*log(h(:,i))-(1-y_vect(:,i))'*log(1-h(:,i)));
end

Theta1Filtered = Theta1(:,2:end); %[25 400]
Theta2Filtered = Theta2(:,2:end); %[10 25]

J = J + lambda/(2*m) * (sum(sum(Theta1Filtered.^2))+sum(sum(Theta2Filtered.^2)));
% -------------------------------------------------------------
...
for t =1:m
    ...
    % step 3
    %          25*10       10*1       25*1
    delta2 = (Theta2Filtered' * delta3).*sigmoidGradient(z2);  % 25*1
    % step 4
    Delta1 = Delta1 + (delta2 * a1);     % 25*1  *  1*401 = 25 * 401
    Delta2 = Delta2 + (delta3 * a2');    % 10*1  *  1*26  = 10 * 26
end
% step 5
